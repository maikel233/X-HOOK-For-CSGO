...
void Autoblock::CreateMove(CUserCmd* cmd)
{
	...
	if (!pInputSystem->IsButtonDown(Settings::Autoblock::key))
		return;
	...
	C_BasePlayer* target = (C_BasePlayer*)pEntityList->GetClientEntity(index);

	if (!target)
		return;

	Vector angles = Math::CalcAngle(localplayer->GetVecOrigin(), target->GetVecOrigin());
