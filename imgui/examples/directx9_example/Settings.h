...
public:
	...
	ImColor Color()
	{
		ImColor result = this->rainbow ? GetRainbowColor(this->rainbowSpeed) : this->color;
...
public:
	...
	{
		ImColor result = this->rainbow ? GetRainbowColor(this->rainbowSpeed) : (this->hp ? Color::ToImColor(GetHealthColor(100)) : this->color);
		result.Value.w = this->color.Value.w;
		return result;
	}
};
...
namespace Settings
{
	...
	namespace Spammer
	{
		...
		namespace KillSpammer
		{
			...
			//		extern bool enabledBombTimerCountdown;
			extern bool sayTeam;
