...
static void DrawPlayer(IMatRenderContext* matctx, const DrawModelState_t& state, const ModelRenderInfo_t& pInfo, matrix3x4_t* pCustomBoneToWorld)
{
	if (!Settings::ESP::Chams::enabled)
		...
		auto records = Backtrack::getRecords(entity->GetIndex());
		if (records && !records->empty() && Backtrack::valid(records->front().simulationTime)) {
			for (int i = 0; i < records->size(); i++) {
				DrawBacktrackedPlayer(matctx, state, pInfo, records->at(i).matrix);
			}
		}
...
static void DrawWeapon(const ModelRenderInfo_t& pInfo) {
	...
	case ArmsType::DEFAULT:
		...
		mat->ColorModulate(Settings::ESP::Chams::Weapon::color.Color().Value.x, Settings::ESP::Chams::Arms::color.Color().Value.y, Settings::ESP::Chams::Arms::color.Color().Value.z);
		break;
	case ArmsType::GLASS:
...
{
	if (!Settings::ESP::enabled && Settings::ESP::Chams::enabled)
		return;
