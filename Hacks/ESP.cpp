...
static void DrawBulletTrace(C_BasePlayer* player) {
	...
	Draw::Line((int)(src.x), (int)(src.y), (int)(dst.x), (int)(dst.y),
		Color::FromImColor(ESP::GetESPPlayerColor(player, true)));
	Draw::FilledRectangle((int)(dst.x - 3), (int)(dst.y - 3), 6, 6,
		Color::FromImColor(ESP::GetESPPlayerColor(player, false)));
}
...
static void DrawPlayer(int index, C_BasePlayer* player, IEngineClient::player_info_t player_info) {
	...
	if (Settings::ESP::ArmorBars::enabled) {
		...
		// clamp it to 100
		int ArmorValue = max(0, min(player->GetArmor(), 100));
