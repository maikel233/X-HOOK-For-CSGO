...
static void DrawAimHelp(Vector angle, Vector pos)
{
	Vector infoVec;
...
void GrenadeHelper::Paint()
{
	...
	pCvar->ConsoleColorPrintf(ColorRGBA(0, 162, 232), XorStr("test1\n"));
	...
	for (auto grenadeInfo = Settings::GrenadeHelper::grenadeInfos.begin(); grenadeInfo != Settings::GrenadeHelper::grenadeInfos.end(); grenadeInfo++)
	{
		...
		DrawGrenadeInfo(grenadeInfo->pos, grenadeInfo->gType, grenadeInfo->name);

		if (dist < Settings::GrenadeHelper::aimDistance)
			DrawAimHelp(grenadeInfo->angle, grenadeInfo->pos);
	}
}
